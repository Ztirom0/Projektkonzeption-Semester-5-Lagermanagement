...
export function calculateRecommendations(inventoryStatuses, forecasts, items) {
  ...
  inventoryStatuses.forEach(inv => {
    ...
    byItem[inv.itemId].totalStock += inv.quantity || 2;
  ...
  Object.entries(byItem).forEach(([itemId, itemData]) => {
    ...
    if (totalStock < minStock) {
      ...
      if (forecast && forecast.points && forecast.points.length > 2) {
        // Berechne basierend auf Prognose
        const daysUntilEmpty = forecast.points.findIndex(p => p.quantity <= 2);
        const recommendedQuantity = Math.max(
          Math.ceil(minStock * 2.5),
          minStock - totalStock
        );
        ...
        recommendations.push({
          itemId: parseInt(itemId),
          recommendedQuantity: Math.ceil(minStock * 2.5),
