...
// Detailansicht eines Produkts: Verkäufe, Bestand, Forecast, Empfehlungen
...
export default function ProductReportView({ item, onBack }) {
  ...
  // Lädt alle relevanten Daten für das ausgewählte Produkt
  useEffect(() => {
    const load = async () => {
      if (!item) return;
      ...
      try {
        // Verkäufe + aktueller Bestand
        const [salesRes, inv] = await Promise.all([
          getSales(),
          getInventory()
        ]);

        // Nur Verkäufe dieses Produkts
        ...
        // Forecast für die nächsten 10 Tage
        const forecastData = [
          calculateForecast(salesRes, history, item.id, forecastMethod, 10)
        ];
        setForecasts(forecastData);

        // Berechneter Bestandsstatus (Tagesverbrauch, Resttage, etc.)
        ...
        // Empfehlungen basierend auf Status + Forecast
  ...
  if (!item) {
    return <div className="alert alert-warning">Kein Produkt ausgewählt.</div>;
  }
  ...
  return (
    <div className="product-report-view">
      ...
      {loading ? (
        ...
        <>
          {/* Chart: Verkäufe, Bestand, Forecast, Empfehlungen */}
          <div className="mb-4">
            <CeoChart
              sales={sales}
                  ...
                  {inventoryStatus ? (
                    <div className="border rounded p-3" style={{ backgroundColor: "#f9f9f9" }}>
                      <div className="fw-bold h5 mb-0">
                        {inventoryStatus.currentQuantity} Stück
                      </div>

                      <div className="small mb-0">
                        <strong>Täglicher Verkauf:</strong>{" "}
                        ~{inventoryStatus.dailySalesRate.toFixed(1)} Stück/Tag
                      </div>

                      <div className="small mb-0">
                        <strong>Verfügbar für:</strong>{" "}
                        ~{inventoryStatus.daysRemaining} Tage
                      </div>
