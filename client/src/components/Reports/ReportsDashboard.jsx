...
useEffect(() => {
  const load = async () => {
    ...
    try {
      ...
      // 5. Forecasts berechnen
      const forecastsData = ids.map(itemId => {
        try {
          return calculateForecast(s, combinedHistory, itemId, "moving-average", 30);
        } catch (err) {
          console.warn(`Forecast failed for item ${itemId}:`, err);
          return null;
        }
      }).filter(f => f !== null);
      setForecasts(forecastsData);

      // 6. Inventory Status laden
      const statusPromises = ids.map(itemId =>
        getInventoryStatus(itemId).catch(err => {
          console.warn(`Status failed for item ${itemId}:`, err);
          return null;
        })
      );

      const statusResults = await Promise.all(statusPromises);
      setInventoryStatuses(statusResults.filter(s => s !== null));
